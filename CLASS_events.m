...
%The constructor for this class takes the EDF index (i.e. chan_num) and
...
% STAGES.cycles field
    properties
        vertical_offset_delta; %specifies the 0 position on the y-axes for the event to use as a reference when plotting
        channel_number; %as seen in the EDF header
        channel_name; %as seen in the EDF header
        class_channel_index; %index into a parent CLASS_channel object that is associating with this event
        samplerate; %sampling rate
        label;
        current_event_index; %the index of this event in terms of the event container object
        start_stop_matrix;
        batch_mode_label;        
        configID; %the configuration identifier (1..M) for this event.  Output from detectors with configurable parameter settings can be uniquely identified in conjunction with this ID and the batch.saveDetectorConfigLegend.m function
        detectorID; % primary key for detectorinfo_t record an instance of this class is associated with
        evt_patch_spacing; %distance between patches/lines in an instance of this class
        evt_patch_y; %y value to draw the event patch at.
        epoch_start_x; %starting sample of the current sev display, where all of the channels and events begin from on the x-axis
        evt_patch_height;
        roc_comparison_range; %range over which to apply an roc comparison to between events - normally for manually loaded events
        parent_axes; %where these things will be plotted too.
        parent_fig;
        evt_patch_h; %vector of handles to lines (one for each source) - defined outside of the class and then assigned directly to this property
        cur_color;
        label_h; %vector of handles to the textboxes that will contain the descriptions of these patches...
        contextmenu_evt_patch_h; %handle to the context menu associated with this event
        hidden; %whether the element is visible or not (patch and labelhandle specifically)
        paramStruct; %this is the parameters of interest for this event that will ilkely be saved later for post analysis
        paramtexthandle; %handle to textbox that displays paramStruct values
        paramFieldIndex; %index of the fieldname to use
        paramFieldNames; %cell of fieldnames in paramStruct
        summary_stats_Struct; %structure of summary statistics calculated in calculate_summary_stats method and returned by get_summary_stats() method
        summary_stats_needs_updating_Bool; %set to true whenever the object's event has been updated so that a new summary stats will be calculated before being returned
        source; %struct that contains the following fields
    ...
    methods(Static)
        ...
        function summary_stats = calculate_summary_stats(samplerate,start_stop_matrix, stage_struct)
            ...
            summary_stats.count.evt_all = all_count; %number of events total
        ...
        function clean_events = cleanup_events(events_in,min_samples)
            %extract only those events (from events_in) that exceed a minimum duration
            %(min_samples)
            %events_in is a two column matrix
            %min_samples is a scalar value
            if(nargin==1)
            ...
            if(~isempty(events_in))
                clean_indices = (events_in(:,2)-events_in(:,1))>min_samples;
                clean_events = events_in(clean_indices,:);
            else
                clean_events = events_in;
            end;
        end
        ...
        function filled_events = fill_bins_with_events(events, num_samples)
            ...
            if(isempty(events)||num_samples<=1)
...
%             start_times = datestr(start_times,'HH:MM:SS.FFF');
            ...
            duration = (events(:,2)-events(:,1))/obj.samplerate;
...
%             y = [duration(:),start_epochs(:),start_stages(:)];
            ...
            save(filename,'events','source','paramStruct','label','duration','start_stages','start_epochs','start_cycle','start_stop_matrix');
        end
...
%                 end
                ...
                duration = (events(:,2)-events(:,1)+1)/obj.samplerate;
                %             y = [obj.start_stop_matrix, duration(:)];
