function MARKING = parfor_batch_process(pathname, BATCH_PROCESS,playlist, MARKING)
    ...
    if(pathname)
        ...
        if(~isfolder(BATCH_PROCESS.output_path.current))
        ...
        if(~isfolder(full_events_images_path))
        ...
        if(~isfolder(full_artifacts_path))
        ...
        if(~isfolder(full_artifacts_images_path))
        ...
        if(BATCH_PROCESS.images.save2img)
            ...
            for k = 1:numel(artifact_settings)
                if(artifact_settings{k}.save2img)
                    ...
                    if(~isfolder(artifact_images_path))
