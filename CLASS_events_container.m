...
%The class events_container_class exists for the purpose of organizing,
%updating, and adjusting instances of CLASS_events.  
...
% 2/6/2013 - added StageStruct here
    properties
        ...
        events_to_plot; %vector containing indices of the events that should be plotted.
...
%                 cur_STA_filename = fullfile(edf_sta_path,STAFiles{s});
                ...
                if(exist(cur_STA_filename,'file'))
                    ...
                    events_container = CLASS_events_container();
                    events_container.setDefaultSamplerate(samplerate);  %this is required to handle the incorporation of new events added from elsewhere

                    for f=1:numel(EvtFiles)
                        evtFile = fullfile(evt_path,EvtFiles{f});
                        cur_evt = events_container.loadEvtFile(evtFile);
                        if(~isempty(cur_evt.events))
                            curEvtObj = events_container.getCurrentChild();
                    ...
                    events_container.save2DB(dbStruct,uniquePat{s},sev_STAGES);                  
                end
            end
        end
    end
    methods
        ...
        function [count, time_in_sec] = events_count(obj,event_index)
             if(event_index>0 && event_index<=obj.num_events)
                 events_matrix = obj.cell_of_events{event_index}.start_stop_matrix;
                 count = size(events_matrix,1);
                 time_in_sec = sum(events_matrix(:,2)-events_matrix(:,1))/obj.cell_of_events{event_index}.samplerate;
        ...
        function obj = addEmptyEvent(obj,event_label,parent_index,sourceStruct,paramStruct)
           ...
           obj.events_to_plot(obj.num_events) = 0;
        ...
        function obj = addEvent(obj,event_data, event_label,parent_index,sourceStruct,paramStruct)
            ...
            if(~isempty(event_data) && all(event_data(:))) %not empty and non-zero indices
                try
                    ...
                    obj.cur_event_index = obj.num_events;
                    obj.events_to_plot(obj.cur_event_index) = 1;
        ...
        function set_Channel_drawEvents(obj,event_index)
            ...
            if(~isempty(eventObj))
                ...
                obj.events_to_plot(event_index) = 1;
                obj.summary_stats_axes_needs_update = true;
                eventObj.draw();
            end
        end
        ...
        function obj = removeEvent(obj,event_index)
            ...
            obj.events_to_plot(event_index) = [];
        ...
        % .algorithm = algorithm name that the event was derived from        
            ...
            if(event_index>0)
                ...
                obj.summary_stats_axes_needs_update = true;
                obj.events_to_plot(event_index) = true;                
            end
        end
        
        %varargin is an optional start stop range that can be used to put
        %bounds on the events under consideration
        function [score, events_space] = compareEvents(obj,event_indices,varargin)
           if(numel(varargin)>0)
               [score, events_space] = compareEvents(...
...
%                 Q(1,4) = TN;
        ...
        function save2mat(obj,filename, markingObj)            
            ...
            if(nargin<2)
                ...
                [events_filename, events_path] = uiputfile('*.*', 'Save Events to .mat format',suggested_filename);
                if isequal(events_filename,0) || isequal(events_path,0)
                    disp('User pressed cancel');
                    filename = [];
                else
                    filename = fullfile(events_path,events_filename);
                    localMARKING.sev.src_event_pathname = events_path;
                end
                    ...
                    if(ishghandle(dlg)) %if it is still a graphic, then...
                        ...
                        if(numel(h_channels)==1)
                            if(get(h_channels,'value'))
                                events2save = get(h_channels,'userdata');
                            else
                                events2save = [];
                            end
                        else
                            events2save = get(h_channels(cell2mat(get(h_channels,'value'))==1),'userdata');
                        end;
                        if(iscell(events2save))
                            events2save = cell2mat(events2save);
                        end
                        ...
                        if(any(events2save))
                            ...
                            if(~isequal(evt_filename,0) && ~isequal(evt_pathname,0))
                                ...
                                for k =1:numel(events2save)
                                    if(filterspec_index==1)
                                        obj.cell_of_events{events2save(k)}.save2text(evt_filename);
                                    elseif(filterspec_index==2)
                                        obj.cell_of_events{events2save(k)}.save2mat(evt_filename);
                                    end
                                end
                            end
                        end
