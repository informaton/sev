% This differs from resizePanelAndFigureForUIControls in the way 'tags' are
% identified.  In this method tag names follow the format <name><numeric suffix>
% Where the resizePanelAndFigureForUIControls tags' have the format
% <name>_<numeric suffix>. 
% A separate function was created to handle this difference two lines
% changed to address different regular expression (i.e. sans '_')
function handles  = resizeBatchPanelAndFigureForUIControls(panel_h,num_rows_desired,handles,optional_vertical_buffer)
%
...
f=regexp(pan_children_tag,'.*(?<suffix>\d+)','names');
...
first_uicontrol_h = findobj(pan_children_h,'-regexp','tag',sprintf('.*%u$',min_suffix));
tags = get(first_uicontrol_h,'tag');
parseStr=regexp(tags,'(?<prefix>.+)\d+','names');
...
if(new_panel_height~=original_panel_height)
    for k =1:max(max_suffix,num_rows_desired)
        for t=1:numel(tag_prefixes)
            ...
            tagStr = sprintf('%s%u',tag_prefixes{t},k);
            tmp_pos(2) = y_offset;
