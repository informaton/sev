...
set(handles.push_psd_settings,'enable','off','userdata',PSD);


if(isfield(DEFAULTS,'batch_folder'))
    if(~isdir(DEFAULTS.batch_folder) || strcmp(DEFAULTS.batch_folder,'.'))
        DEFAULTS.batch_folder = pwd;
    end;
    set(handles.edit_edf_directory,'string',DEFAULTS.batch_folder);
else
...
if(resized_panel_h==handles.panel_psd)
    ...
    h_params = uicontrol(GUI_TEMPLATE.push_parameter_settings,'parent',resized_panel_h,'userdata',PSD);
...
if(isfield(DEFAULTS,'detection_path'))
    detection_inf = fullfile(DEFAULTS.detection_path,'detection.inf');
else
...
% BATCH_PROCESS.output_files =
%           psd_filename: 'psd.txt'
%                music_filename: 'MUSIC'
%               events_filename: 'evt.'
%            artifacts_filename: 'art.'
%                      save2mat: 0
%                       save2DB: 0
%     cumulative_stats_checkbox: 0
%     cumulative_stats_filename: 'SEV_cumulative_stats.txt'
%                  log_checkbox: 1
%                  log_filename: '_log.txt'
%         images_save_directory: 'images'
%                 images_format: 'PNG'
...
%     file_list = dir([fullfile(path, '*.EDF');fullfile(path, '*.edf')]);
    ...
    STATE.batch_process_running = true;
                                    ...
                                    if(end_evt_ind>=start_evt_ind)  %make sure I didn't go through and get nothing...
                                        ...
                                        if(~isempty(truth_file))
                                            ...
                                            if(exist(truth_filename,'file'))
                                                ...
                                                batch_EVENT_CONTAINER.loadEvtFile(truth_filename,STATE.batch_process_running);
...
%     waitHandle = findobj('tag','waitbarTag');
    ...
    STATE.batch_process_running = false;
...
%         num_files_attempted,num_files_completed,files_skipped);
    ...
    if(num_files_skipped>0)
        ...
        [selections,clicked_ok]= listdlg('PromptString',message,'ListString',skipped_filenames,'Name','Batch Completed',...
            'OKString','Copy to Clipboard','CancelString','Close','ListSize',[160*2,numel(skipped_filenames)*16]);
