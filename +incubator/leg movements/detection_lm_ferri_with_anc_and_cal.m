function detectStruct = detection_lm_ferri_with_anc_and_cal(channel_indices, optional_params)
%modifies detection_lm_ferri_with_anc.  Now goes back over the ECG channel
%at the obtained LMs and removes any LM's not associated with
%cardiovascular variability as obtained from analyzing HR 10seconds prior
%to and 10-s after each LM.  cal for cardiovascular arousal lock (cal)
% modification of detection_lm_ferri.  Now takes a second channel, the ECG
% channel and performs adaptive noise cancelling first before applying leg
...
global DEFAULTS;
tic
% LEG_channel = CHANNELS_CONTAINER.cell_of_channels{channel_indices(1)};
% ECG_channel = CHANNELS_CONTAINER.cell_of_channels{channel_indices(2)};

leg_index = channel_indices(1);
ecg_index = channel_indices(2);
sample_rate = CHANNELS_CONTAINER.getSamplerate(leg_index);
...
if(nargin==2 && ~isempty(optional_params))
    ...
    pfile = '+detection/detection_lm_ferri_with_anc_and_cal.plist';
...
%         params.filter_order = 50;%1/2 second worth at 100Hz sampling rate
        ...
        params.upper_threshold_uV = 10;  %8 uV above resting - presumably resting is 2uV
...
LM_line = abs(filter.anc_rls(leg_index,ecg_index));
...
%these parameters will store the cal values that I am interested in.
HR_before = zeros(size(LM));
HR_after = HR_before;
...
paramStruct.HR_before = HR_before;
paramStruct.HR_after = HR_after;
paramStruct.AUC = AUC;
