function detectStruct = detection_lm_ferri_with_anc(channel_indices, optional_params)
...
%     pfile = '+detection/detection_lm_ferri_with_anc.plist';
...
%do I need to reapply the outer LM duration?
...
%have to take into account start and stops being in different epochs
%so I make it a 2-column matrix.
...
%increase specificity this way...
exclude_indices = AUC<5;
AUC = AUC(~exclude_indices);
LM = LM(~exclude_indices,:);
detectStruct.new_events = LM;
paramStruct.AUC = AUC;
paramStruct.density = paramStruct.AUC./(LM(:,2)-LM(:,1))*sample_rate; %uV/sec
