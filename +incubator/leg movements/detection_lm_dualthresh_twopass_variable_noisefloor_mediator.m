function detectStruct = detection_lm_dualthresh_twopass_variable_noisefloor_mediator(channel_indices, optional_params)
...
% dur_samples_below_count = ceil(params.min_duration_sec*samplerate*0.05);

dur_samples_below_count = ceil(0.05*samplerate);
% dur_samples_below_count = ceil(0.5*samplerate);
% dur_samples_below_count = ceil(params*samplerate);
...
%     secondpass_events = variable_dualthresholdcrossings(clean_data2, variable_threshold_high_uv,variable_threshold_low_uv,dur_samples_below_count);
...
%     
%     for n=1:num_events
%         datum = abs(data(new_events(n,1):new_events(n,2)));
%         if(median(datum)<variable_threshold_low_uv(round(new_events(n,1)+(new_events(n,2)-new_events(n,1))/2)))
%             held_events(n) = false;
%         end
%     end
...
%     new_events(:,1)  = new_events(:,1)+ceil(ma.params.order/4);
