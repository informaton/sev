...
% 
% handles.user.axes1.y_max = 300;
% handles.user.axes1.y_min = -300;
% ylim(handles.axes1,[-300 300]);


% set(handles.sev_main_fig,'renderer','OpenGL');  %this gives a funny little
% tick at the bottom of axes1 - so I am removing it

%adjust these for backward compatibility with older MATLAB versions

% set(handles.sev_main_fig,'KeyPressFcn',@sev_main_fig_KeyPressFcn);
% set(handles.sev_main_fig,'KeyReleaseFcn',@sev_main_fig_KeyReleaseFcn);

% set(handles.sev_main_fig,'KeyPressFcn',@figure1_KeyPressFcn);
% set(handles.sev_main_fig,'KeyReleaseFcn',@figure1_KeyReleaseFcn);

%set default values

handles.user.annotationH = -1;

%set the value found in the uicontext menu as established/set with GUIDE
%(in the sev.fig) - determine the default value and assign it here - may
%want to improve this by saving this parameter to the text file
% handles.user.analysis.spacing = str2double(get(findobj(allchild(handles.context_menu_label_spacing),'checked','on'),'label'));
% commented out on 5/16/2011
