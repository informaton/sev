...
classdef CLASS_SSC_database < CLASS_database
    ...
    methods
        ...
        function create_Diagnostics_T(obj, diagnostics_xls_filename)
            ...
            % Edited:
            %   8/31/12 - using new file from Laurel at SSC as input.
            ...
            %   7/26-7/27/12 - incorporate medication fields received from SSC -
            ...
            %   convertSNPFile2SSCFile.m script, which changes names of PatID's and
            ...
            if(nargin<2 || isempty(diagnostics_xls_filename))
                [wsc_filename, pathname, ~] = uigetfile({'*.xls','Microsoft Excel (*.xls)';'*.xlsx','Microsoft Excel (*.xlsx)'},'Select SSC Diagnostics File');
        ...
        %> SSC.  
        %> @param obj CLASS_SSC_database instance
        ...
        function create_Blood_T(obj,blood_xls_filename)
            ...
            if(nargin<2 || isempty(diagnostics_xls_filename))
                [blood_xls_filename, pathname, filterindex] = uigetfile({'*.xls','Microsoft Excel (*.xls)';'*.xlsx','Microsoft Excel (*.xlsx)'},'Select SSC Blood data file');
            ...
            %             xls_filename = 'all_available_SSC_with_patids_testdates_and_priority-ling1-jl-12-10.xls'; %provided by Jason on 12/12/2012
        ...
        %> polymorphisms (SNPs) provided in the filenames for SSC subjects.
        %> @param obj CLASS_database derived instance
        %> @param snp_filenames_cell Cell of filenames (.txt) with SSC data
        %> (cell of strings)
        % =================================================================
        function update_Diagnostics_T_for_SNP(obj,snp_filenames_cell)
            ...
            if(nargin==1 || isempty(snp_filenames_cell))
                [snp_filenames_cell, pathname, ~] = uigetfile({'*.txt','Tab-delimited Text (*.txt)'},'Select SSC SNP data file(s)','MultiSelect','on');
...
%                 %     snp_filenames_cell = {'wsc_snp_rs11693221_corrected.txt'};
            ...
            for s=1:numel(snp_filenames_cell)
                ...
                if(exist(snp_filename,'file'))
                    ...
                    q = mym(sprintf(['SELECT count(*) as count FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = "SSC_DB"'...
        ...
        %> obtained automatically for a given detector applied to SSC studies
        %> @param obj CLASS_database derived instance
        ...
        % =================================================================
        function update_SSC_Diagnostics_T_for_PLM(obj,detector,resp)
            if(mym)
                obj.open();
            end
            if(nargin==1)
    ...
    methods(Static)
        ...
        % @brief Export parts of SSC Diagnostics_T to tab delimited text
        % file
