function metrics = calculatePhasicEventMetrics(mat_events_epoch_cycle, stage_dur_hour,mat_resp,sample_rate,key, plm_params,respiratoryRemovalType)
...
%metrics is a structure with the following fields
%     metrics.periodicity = periodicity_results;
%     metrics.hrs_evaluated = stage_dur_hour;
%     metrics.attrition_strength = strength_attrition_results;
%     metrics.attrition_plm = plm_attrition_results;
%     metrics.strength =strength_results;
%     metrics.PLMI = PLMI_results;
%     metrics.HR_delta = change in heart rate from min to max
%     metrics.HR_slope = slope of heart rate change

% Written by Hyatt Moore IV
% for collaboration with VA on December 5, 2012
if(nargin==0)
    metrics.emcount = 'Eye movement count';
    metrics.strength = 'Strength';
    metrics.attrition_em_by_hour = 'EM attrition by hour;
    metrics.attrition_em_by_cycle = 'EM attribtion by NREM/REM cycle';
    metrics.hrs_evaluated = 'Hours evaluated';
else
    metrics = [];
    ...
    if(LM_count>2) %otherwise, may have a divide by 0 for ferri's periodicity calculation
        ...
        if(stage_dur_hour>0)
            ...
            metrics.PLMI = PLMI_results;
            metrics.lmcount = LM_count;
            metrics.periodicity = periodicity_results;
            metrics.strength =strength_results;
            metrics.attrition_plm_by_hour = plm_attrition_by_hour_results;
            metrics.attrition_plm_by_cycle = plm_attrition_by_cycle_results;
            metrics.attrition_lm_by_hour = lm_attrition_by_hour_results;
            metrics.attrition_lm_by_cycle = lm_attrition_by_cycle_results;
            
            metrics.attrition_strength = strength_attrition_results;
            metrics.hrs_evaluated = stage_dur_hour;
            metrics.HR_delta = mean(HR_delta);
            metrics.HR_slope = mean(HR_slope);
            
            metrics.ratio_plm = ratio_plm;
            metrics.ratio_lm = ratio_lm;
        end
    end
end
